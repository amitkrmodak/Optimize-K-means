...
def cal_ccpi(c1:list, c2:list):
    ...
    for s in range(k):
        for j in range(2):
            #print(s,j, c1, c2)
            # sum+=abs((c2[s][j]-c1[s][j])/(c2[s][j] if c2[s][j] != 0 else 1e-10))
            sum += abs((c2[s][j] - c1[s][j]) / c2[s][j])
    return (sum/div)
# def cal_ccpi(c1, c2):
#     sum=0.0
...
print("Centroids generated using max Algorithm =", centroid)
centroid_x = []
centroid_y = []
...
########################################### BCSS #######################################
i = 1
with open('kmeansC_output_max.txt', 'w') as f:
    f.write("\nLength =")
...
plt.title("Standard K-means Clustering (max)")
plt.xlabel('x -->')
plt.ylabel('y -->')
plt.show()
